...
This node controls the FOCUS TASK (WITHOUT visual gaze position feedback) and 
    position of the focus circle target in unity through service calls. 
"""
import numpy as np
...
class UnityFocusController(Node):
    def __init__(self, training_flag):
        super().__init__('unity_focus_controller')
    ...
    def __load_targets(self):
        if "true" in self.training_flag.lower():
            ...
            self.targets_fname = "focus_targets_random_1.csv" 
        else:
    ...
    def pause_callback(self, request, response):
        ...
        self.get_logger().info("PAUSE GAME requested")
        return response

    def resume_callback(self, request, response):
        self.game_paused = False
        self.event_msg_publisher("resume")
        self.get_logger().info("RESUME GAME requested")
        return response 
    ...
    def main_timer_callback(self):
        ...
        if not self.game_paused:
            if self.new_target_flag:
                ...
                self.get_logger().info("current target is: {}; {}; radius: {}".format(self.current_target_idx, self.current_target, self.target_radius))
    ...
    def __check_gaze_target_overlap(self):
        ...
        if gaze_dist_from_target <= self.target_radius:
            ...
            if self.overlap_flag:
                ...
                ## set this back to 0 once the gaze leaves the target 
                self.dwell_time = 0.0

        ## setting alpha value here         
